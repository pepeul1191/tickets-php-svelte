...
class PositionController extends BaseController
{
  ...
  function list($f3)
  {
    ...
    // logic
    try {
      $rs = \Model::factory('App\\Models\\Position', 'app')
        ->find_array();
      $resp = json_encode($rs);
    }catch (\Exception $e) {
      $status = 500;
  ...
  function save($f3)
  {
    ...
    try {
      ...
      if(count($news) > 0){
				foreach ($news as &$new) {
				  $n = \Model::factory('App\\Models\\Position', 'app')->create();
					$n->name = $new['name'];
					$n->save();
				  $temp = [];
				  $temp['tempId'] = $new['id'];
      ...
      if(count($edits) > 0){
          ...
          $e = \Model::factory('App\\Models\\Position', 'app')->find_one($edit['id']);
					$e->name = $edit['name'];
					$e->save();
        }
      }
      ...
      if(count($deletes) > 0){
			    ...
			    $d = \Model::factory('App\\Models\\Position', 'app')->find_one($delete['id']);
			    $d->delete();
				}
      }
      // commit
